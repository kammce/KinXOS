    ...
    *  backcolor too */
    ...
    if(csr_y >= 24)
    {
        ...
        *  back in the buffer by a line */
        temp = csr_y - 24;
        //Destination memory from the buffer from an offset of 80 (a whole row)
        //Source memory is the same buffer but offset 2 rows below
        //Copy by the count of 23 rows by 80 colums *2 (*2 because we need to clear the attribute and the character)
        memcpy (textmemptr+80, textmemptr + (80*2), 23 * 80 * 2);

        /* Finally, we set the chunk of memory that occupies
        *  the last line of text to our 'blank' character */
        memsetw(textmemptr + (23 * 80), blank, 80);
        csr_y = 25 - 2;
    }
}
...
void puti( int num ) {
    itoa(strnum, num);
    puts(strnum);
    for(counter_0 = 0; counter_0 < 50; counter_0++) {
        strnum[counter_0] = '\0';
    }
}
